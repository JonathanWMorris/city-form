...
bool FCityFormCameraTuning::IsValid(FString* OutError) const
{
	...
	if (!FMath::IsFinite(MinimumPanSpeed) || !FMath::IsFinite(MaximumPanSpeed) ||
		MinimumPanSpeed < 0.0 || MaximumPanSpeed < MinimumPanSpeed)
	{
	...
	if (!FMath::IsFinite(MinimumZoom) || !FMath::IsFinite(MaximumZoom) || !FMath::IsFinite(InitialZoom) ||
		MinimumZoom <= 0.0 || MaximumZoom < MinimumZoom ||
		InitialZoom < MinimumZoom || InitialZoom > MaximumZoom)
	{
	...
	if (!FMath::IsFinite(MinimumPitch) || !FMath::IsFinite(MaximumPitch) || !FMath::IsFinite(InitialPitch) ||
		MinimumPitch < -89.0 || MaximumPitch > -1.0 || MinimumPitch > MaximumPitch ||
		InitialPitch < MinimumPitch || InitialPitch > MaximumPitch)
	{
	...
	if (!FMath::IsFinite(InitialYaw) || !FMath::IsFinite(ScrollZoomStep) || !FMath::IsFinite(KeyboardZoomSpeed) ||
		...
		!FMath::IsFinite(PointerOrbitSensitivity) || !FMath::IsFinite(SmoothingResponse) ||
		ScrollZoomStep < 0.0 || KeyboardZoomSpeed < 0.0 || KeyboardYawSpeed < 0.0 ||
		KeyboardPitchSpeed < 0.0 || PointerOrbitSensitivity < 0.0 || SmoothingResponse < 0.0 ||
