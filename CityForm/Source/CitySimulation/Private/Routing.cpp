...
struct FOpenEntryWorse
{
	...
	{
		if (Left.PredictedTotalMilliseconds !=
			Right.PredictedTotalMilliseconds)
		{
			return Left.PredictedTotalMilliseconds >
				Right.PredictedTotalMilliseconds;
		}
...
{
	return VehicleClass.Id.IsValid() &&
		std::isfinite(VehicleClass.MaximumSpeedMetersPerSecond) &&
		VehicleClass.MaximumSpeedMetersPerSecond > 0.0;
}

FRouteError ComputeHeuristic(
	...
	int64& OutMilliseconds)
{
	...
	{
		return {
			ERouteErrorCode::InvalidGraph,
			TEXT("The routing heuristic could not resolve a graph node.")};
	}

	const double DistanceMeters = std::hypot(
		Destination->PositionMeters.X - Node->PositionMeters.X,
		Destination->PositionMeters.Y - Node->PositionMeters.Y);
	const double Milliseconds =
		DistanceMeters / VehicleClass.MaximumSpeedMetersPerSecond * 1000.0;
	if (!std::isfinite(Milliseconds) ||
		Milliseconds < 0.0 ||
		Milliseconds >= static_cast<double>(MAX_int64))
	{
		return {
			ERouteErrorCode::InvalidTraversalCost,
			TEXT("The routing heuristic produced an invalid duration.")};
	}
...
bool IsPreferredPredecessor(
	const FRoadTraversal& Candidate,
	const TOptional<FRoadTraversal>& Existing)
{
	if (!Existing.IsSet())
	{
		return true;
...
bool WouldCreatePredecessorCycle(
	const FRoadTraversal& Candidate,
	const TMap<FRoadNodeId, FNodeRouteState>& States)
{
...
bool HasTopologicalPath(
	const FRoadGraph& Graph,
	const FRoadNodeId OriginNodeId,
	const FRoadNodeId DestinationNodeId)
{
	...
	while (!Pending.IsEmpty())
	{
		...
		for (const FRoadTraversal& Traversal :
			Graph.GetOutgoingTraversals(CurrentNodeId))
		{
...
FRouteResult ReconstructRoute(
	...
	const TMap<FRoadNodeId, FNodeRouteState>& States)
{
	const FNodeRouteState* DestinationState =
		States.Find(Query.DestinationNodeId);
	if (DestinationState == nullptr || !DestinationState->bReached)
	{
		return Failure(
			ERouteErrorCode::Disconnected,
			TEXT("The destination is disconnected from the origin."));
	}

	FRoute Route;
	Route.ArrivalInstant =
		FSimulationInstant(DestinationState->ArrivalMilliseconds);
	Route.TravelDuration = FSimulationDuration(
		DestinationState->ArrivalMilliseconds -
		Query.DepartureInstant.GetMillisecondsSinceStart());
	...
	while (CurrentNodeId != Query.OriginNodeId)
	{
		...
		if (State == nullptr || !State->Predecessor.IsSet())
		{
			return Failure(
				ERouteErrorCode::InvalidGraph,
				TEXT("Route reconstruction found a missing predecessor."));
		}
		...
		if (Segment == nullptr)
		{
			return Failure(
				ERouteErrorCode::InvalidGraph,
				TEXT("Route reconstruction found a missing road segment."));
		}
		...
		if (Route.NodeIds.Num() > Graph.GetNodes().Num())
		{
			return Failure(
				ERouteErrorCode::InvalidGraph,
				TEXT("Route reconstruction detected a predecessor cycle."));
		}
	}
...
FTraversalCostResult FTraversalCostResult::Failure(
	const ERouteErrorCode Code,
	FString Message)
{
	FTraversalCostResult Result;
...
FTraversalCostResult FFreeFlowTraversalCostProvider::Evaluate(
	const FRoadGraph& Graph,
	...
	const FVehicleClassDefinition& VehicleClass) const
{
	const FRoadSegment* Segment = Graph.FindSegment(Traversal.SegmentId);
	if (Segment == nullptr ||
		(Traversal.FromNodeId != Segment->EndpointA &&
			Traversal.FromNodeId != Segment->EndpointB) ||
		(Traversal.ToNodeId != Segment->EndpointA &&
			Traversal.ToNodeId != Segment->EndpointB) ||
		Traversal.FromNodeId == Traversal.ToNodeId)
	{
		return FTraversalCostResult::Failure(
			ERouteErrorCode::InvalidGraph,
			TEXT("A traversal must reference a segment and its two endpoints."));
	}

	const FRoadSpeedLimitResult SpeedLimit =
		Graph.ResolveSpeedLimit(*Segment, RoadTypes);
	if (!SpeedLimit.IsSuccess())
	{
		return FTraversalCostResult::Failure(
			ERouteErrorCode::InvalidTraversalCost,
			TEXT("The traversal speed limit could not be resolved."));
	}

	const double EffectiveSpeedMetersPerSecond = FMath::Min(
		SpeedLimit.SpeedLimitMetersPerSecond,
		VehicleClass.MaximumSpeedMetersPerSecond);
	const double Milliseconds =
		Segment->LengthMeters / EffectiveSpeedMetersPerSecond * 1000.0;
	if (!std::isfinite(Milliseconds) ||
		Milliseconds <= 0.0 ||
		Milliseconds >= static_cast<double>(MAX_int64))
	{
	...
	return {
		FSimulationDuration(
			FMath::Max<int64>(
				1,
				static_cast<int64>(std::ceil(Milliseconds)))),
		ETraversalCostStatus::Success,
		{}};
}

FRouteResult FTimeDependentRouter::FindRoute(
	const FRoadGraph& Graph,
	const FRoadTypeCatalog& RoadTypes,
	...
	const FRouteOptions Options)
{
	...
	{
		return Failure(
			ERouteErrorCode::InvalidOrigin,
			TEXT("The route origin must reference an existing road node."));
	}
	if (Graph.FindNode(Query.DestinationNodeId) == nullptr)
	{
		return Failure(
			ERouteErrorCode::InvalidDestination,
			TEXT("The route destination must reference an existing road node."));
	}
	if (Query.DepartureInstant.GetMillisecondsSinceStart() < 0)
	{
		return Failure(
			ERouteErrorCode::InvalidDepartureTime,
			TEXT("The route departure instant cannot be negative."));
	}

	const FVehicleClassDefinition* VehicleClass =
		VehicleClasses.Find(Query.VehicleClassId);
	if (VehicleClass == nullptr || !IsValidVehicleClass(*VehicleClass))
	{
		return Failure(
			ERouteErrorCode::InvalidVehicleClass,
			TEXT("The route must reference a valid VehicleClass."));
	}
	...
	if (OriginHeuristic >
		MAX_int64 - OriginState.ArrivalMilliseconds)
	{
		return Failure(
			ERouteErrorCode::TimeOverflow,
			TEXT("A* predicted total time exceeds the simulation range."));
	}

	std::priority_queue<
		FOpenEntry,
		std::vector<FOpenEntry>,
		FOpenEntryWorse> Open;
	Open.push({
		Query.OriginNodeId,
		OriginState.ArrivalMilliseconds,
		OriginHeuristic,
	...
	while (!Open.empty())
	{
		...
		if (CurrentState == nullptr ||
			Current.ArrivalMilliseconds !=
				CurrentState->ArrivalMilliseconds)
		{
			continue;
		}
		...
		if (DestinationState != nullptr &&
			DestinationState->bReached &&
			Current.PredictedTotalMilliseconds >
				DestinationState->ArrivalMilliseconds)
		{
			break;
		}

		const TArray<FRoadTraversal> Traversals =
			Graph.GetOutgoingTraversals(Current.NodeId);
		for (const FRoadTraversal& Traversal : Traversals)
		{
			const FTraversalCostResult Cost = CostProvider.Evaluate(
				...
				RoadTypes,
				Traversal,
				FSimulationInstant(Current.ArrivalMilliseconds),
				*VehicleClass);
			...
			if (!Cost.IsSuccess() ||
				Cost.Duration.GetMilliseconds() < 0)
			{
				return Failure(
					Cost.Error.IsSet()
						? Cost.Error.Code
					...
					Cost.Error.IsSet()
						? Cost.Error.Message
						: TEXT("A traversal cost must be non-negative."));
			}

			const FSimulationInstantResult Arrival =
				AddSimulationDuration(
					FSimulationInstant(Current.ArrivalMilliseconds),
					Cost.Duration);
			if (!Arrival.IsSuccess())
			{
				return Failure(
					ERouteErrorCode::TimeOverflow,
					TEXT("Route arrival time exceeds the simulation range."));
			}

			FNodeRouteState* ExistingState =
				States.Find(Traversal.ToNodeId);
			const int64 CandidateArrival =
				Arrival.Instant.GetMillisecondsSinceStart();
			const bool bBetterArrival =
				ExistingState == nullptr ||
				!ExistingState->bReached ||
				CandidateArrival < ExistingState->ArrivalMilliseconds;
			const bool bPreferredEqualArrival =
				ExistingState != nullptr &&
				ExistingState->bReached &&
				CandidateArrival == ExistingState->ArrivalMilliseconds &&
				IsPreferredPredecessor(
					Traversal,
					ExistingState->Predecessor);
	...
	if (DestinationState == nullptr || !DestinationState->bReached)
	{
		const bool bTopologicallyConnected = HasTopologicalPath(
			Graph,
			Query.OriginNodeId,
			Query.DestinationNodeId);
		return Failure(
			bTopologicallyConnected
				? ERouteErrorCode::NoPermittedRoute
				: ERouteErrorCode::Disconnected,
			bTopologicallyConnected
				? TEXT("No permitted route connects the origin and destination.")
				: TEXT("The destination is disconnected from the origin."));
	}
